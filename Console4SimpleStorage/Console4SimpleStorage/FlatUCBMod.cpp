...
void FlatUCBMod::startThreads(int UCTPlayer, GameState gameState, std::vector<Move> moveHistory)
{
	std::vector<std::thread> threadVector;
	for (int counter = 0; counter < UCBMOD_THREADS; counter++)
	{
		threadVector.push_back(std::thread(&FlatUCBMod::doUCT, this, UCTPlayer, gameState, moveHistory));
...
Node* FlatUCBMod::select(Node* root)
{
...
void FlatUCBMod::rollout(Node* node, GameState gameState, int UCTPlayer)
{
	...
	if (gameState.playerStates.size() == 2)
	{
		...
		score = UCBMOD_PERCFACTOR*(gameState.playerStates[UCTPlayer].calculateVictoryPoints() - gameState.playerStates[otherPlayer].calculateVictoryPoints()) / 100.;
		score += ((gameState.playerStates[UCTPlayer].calculateVictoryPoints() > gameState.playerStates[otherPlayer].calculateVictoryPoints()) ? UCBMOD_WINPOINT : UCBMOD_LOSEPOINT);
...
Node* FlatUCBMod::UCTSelectChild(Node* root)
{
	if (root->isRoot)
	{
		for (int i = 0; i < root->childrenPtrs.size(); i++)
		{
			if (root->childrenPtrs[i]->visited < UCBMOD_MINIMUMVISITS)
	...
	for (int i = 0; i < root->childrenPtrs.size(); i++)
	{
		...
		if (root->childrenPtrs.at(i)->opt.type == DRAW) // Do not use value, but probability, for drawnodes.
			value = double(root->childrenPtrs.at(i)->probability) + UCBMOD_C * sqrt(log(double(root->visited) / root->childrenPtrs.at(i)->visited));	// Can have a different C here if necessary.
		else
			value = double(root->childrenPtrs.at(i)->value) + UCBMOD_C * sqrt(log(double(root->visited) / root->childrenPtrs.at(i)->visited));
...
void FlatUCBMod::playActionCard(GameState &gameState, int absoluteCardId, int playerIndex, bool rollout)
{
...
void FlatUCBMod::createAllChildren(Node* node)
{
	...
	else
	{
		...
		if (UCBMOD_PLAYPLUSACTIONSFIRST && node->currentState.playerStates[currentlyPlaying].actions > 0)
		{
...
void FlatUCBMod::createTrashNodes(Node* parentNode, GameState& currentState, int currentlyPlaying)
{
...
Node* FlatUCBMod::getRandomNode(std::vector<Node*> nodes)
{
...
std::vector<Node*> FlatUCBMod::getUntriedChildren(Node* parent)
{
...
int FlatUCBMod::getCurrentMoney(GameState* gameState, int playerIndex)
{
	int currentMoney = 0;
...
std::vector<Option> FlatUCBMod::getBuyOptions(GameState* gameState, int currentMoney)
{
...
std::vector<Option> FlatUCBMod::getActionOptions(GameState* gameState, const int(&hand)[INSUPPLY])
{
...
Option FlatUCBMod::getCardPlayoutPolicy(GameState& gameState, int playerIndex)
{
	bool randomPlayout = false;
	if (UCBMOD_PLAYOUTPOLICY == UCBMOD_EPSILONGREEDY)
	{
		int randomCounter = rand() % 100 + 1;
		if (UCBMOD_EPSILON >= randomCounter)
			randomPlayout = true;
	}
	else if (UCBMOD_PLAYOUTPOLICY == UCBMOD_RANDOMPLAYOUT)
	{
		randomPlayout = true;
	}
	else if (UCBMOD_PLAYOUTPOLICY == UCBMOD_HEURISTICGREEDY)
	{
		randomPlayout = false;
	}
	else
...
{
	nodeAllocationPtr = new Node[UCBMOD_NODESTOALLOCATE];
	handedOutNodes = 0;
	for (int counter = 0; counter < UCBMOD_NODESTOALLOCATE; counter++)
	{
...
Node* FlatUCBMod::requestNewNode()
{
	mtx.lock();
	if (handedOutNodes >= UCBMOD_NODESTOALLOCATE)
	{
...
void FlatUCBMod::resetMyNodes(Node* rootNode)
{
...
void FlatUCBMod::addToReturnVector(std::vector<Node*> nodes)
{
	returnMtx.lock();
...
unsigned long long FlatUCBMod::choose(unsigned long long n, unsigned long long k)
{
	if (k > n) {
		return 0;
	}
